...
(e.g., `status`, `labels`, `assignee`). See the Collection Info section below.
...
2. **Filter Groups** (`filter_groups`): Groups of conditions to narrow the search space.
   ...
   *Base fields:*
   ...
   - `breadcrumbs`: Filter by location hierarchy - useful for finding siblings or parents
   ...
   *System metadata:*
   ...
   - `original_entity_id`: Find all chunks from the same original entity
   - `chunk_index`: Target specific chunk positions
