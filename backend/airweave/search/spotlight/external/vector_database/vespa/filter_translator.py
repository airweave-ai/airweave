"""Filter translator for SpotlightPlan filter_groups → Vespa YQL.

Converts spotlight filter groups to Vespa YQL WHERE clause components.
...
from airweave.search.spotlight.schemas.filter import (
...
class FilterTranslator:
    """Translates SpotlightPlan filter_groups to Vespa YQL.

    Handles the conversion of SpotlightFilterGroup/SpotlightFilterCondition format
    ...
    def translate(self, filter_groups: List[SpotlightFilterGroup]) -> Optional[str]:
        """Translate filter groups to YQL WHERE clause.

        Args:
            filter_groups: List of SpotlightFilterGroups from the plan.
    ...
    def _translate_group(self, group: SpotlightFilterGroup) -> Optional[str]:
        """Translate a single FilterGroup to YQL (AND of conditions).

        Args:
            group: SpotlightFilterGroup with list of conditions.
    ...
    def _translate_condition(self, condition: SpotlightFilterCondition) -> str:
        ...
        Args:
            condition: SpotlightFilterCondition with field, operator, value.
    ...
    def _dispatch_operator(self, field: str, operator: SpotlightFilterOperator, value: Any) -> str:
        ...
        comparison_ops = {
            SpotlightFilterOperator.GREATER_THAN: ">",
            SpotlightFilterOperator.LESS_THAN: "<",
            SpotlightFilterOperator.GREATER_THAN_OR_EQUAL: ">=",
            SpotlightFilterOperator.LESS_THAN_OR_EQUAL: "<=",
        }
        ...
        method_map = {
            SpotlightFilterOperator.EQUALS: self._build_equals,
            SpotlightFilterOperator.NOT_EQUALS: self._build_not_equals,
            SpotlightFilterOperator.CONTAINS: self._build_contains,
            SpotlightFilterOperator.IN: self._build_in,
            SpotlightFilterOperator.NOT_IN: self._build_not_in,
        }
