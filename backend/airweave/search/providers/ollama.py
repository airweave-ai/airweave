...
Supports local Llama models via Ollama.
Uses OpenAI-compatible API for text generation and structured output.
Supports embeddings with nomic-embed-text or other embedding models.
"""
...
class OllamaProvider(BaseProvider):
    ...
    MAX_STRUCTURED_OUTPUT_TOKENS = 4000
    ...
    def __init__(
        self, base_url: str, model_spec: ProviderModelSpec, ctx: ApiContext
    ) -> None:
        """Initialize Ollama provider with model specs from defaults.yml.
        ...
        Args:
            ...
            ctx: API context
        """
        # Pass empty string as api_key since Ollama doesn't require authentication
        super().__init__("not-needed", model_spec, ctx)
        ...
        if model_spec.llm_model:
            self.ctx.logger.info(
                f"[OllamaProvider] Using LLM model: {model_spec.llm_model.name}"
            )
        if model_spec.embedding_model:
            self.ctx.logger.info(
                f"[OllamaProvider] Using embedding model: {model_spec.embedding_model.name} "
                f"({model_spec.embedding_model.dimensions}-dim)"
            )

        self.ctx.logger.debug(f"[OllamaProvider] Initialized with model spec: {model_spec}")
        self.ctx.logger.info(f"[OllamaProvider] Connected to Ollama at {base_url}")
    ...
    async def generate(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Ollama completion API call failed: {e}") from e

        content = response.choices[0].message.content
        if not content:
            raise ValueError("Ollama returned empty completion content")
    ...
    async def structured_output(
        ...
        """Generate structured output using Ollama JSON mode."""
        if not self.model_spec.llm_model:
            raise RuntimeError("LLM model not configured for Ollama provider")
        ...
        # Add schema to the system message for better JSON compliance
        schema_str = json.dumps(schema.model_json_schema(), indent=2)
        enhanced_messages = messages.copy()

        # Find system message and append schema, or create new system message
        system_found = False
        ...
        try:
            ...
            raise RuntimeError(f"Ollama structured output API call failed: {e}") from e

        content = response.choices[0].message.content
        if not content:
            raise RuntimeError("Ollama returned empty structured output content")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to parse Ollama structured output: {e}") from e
    ...
    async def embed(self, texts: List[str]) -> List[List[float]]:
        ...
        if not self.model_spec.embedding_model:
            raise RuntimeError("Embedding model not configured for Ollama provider")
    ...
    def _validate_embed_inputs(self, texts: List[str]) -> None:
        ...
        if not self.embedding_tokenizer:
            raise RuntimeError("Embedding tokenizer not initialized for Ollama provider")
    ...
    async def _embed_batch(self, batch: List[str], batch_start: int) -> List[List[float]]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Ollama embeddings API call failed at index {batch_start}: {e}"
            ) from e

        if not response.data:
            raise RuntimeError(f"Ollama returned no embeddings for batch at {batch_start}")

        if len(response.data) != len(batch):
            raise RuntimeError(
                f"Ollama returned {len(response.data)} embeddings but expected {len(batch)}"
            )

        return [item.embedding for item in response.data]

    async def rerank(self, query: str, documents: List[str], top_n: int) -> List[Dict[str, Any]]:
        """Rerank documents using Ollama structured output."""
        from airweave.search.prompts import RERANKING_SYSTEM_PROMPT

        if not self.model_spec.rerank_model:
            raise RuntimeError("Rerank model not configured for Ollama provider")
        ...
        if not rerank_result.rankings:
            raise RuntimeError("Ollama returned empty rankings")
    ...
    def _budget_documents_for_reranking(
        ...
        if not self.rerank_tokenizer:
            raise RuntimeError(
                ...
                "Ensure tokenizer is configured in defaults.yml for Ollama rerank model."
            )

        if not self.model_spec.rerank_model or not self.model_spec.rerank_model.context_window:
            raise RuntimeError("Context window required for Ollama reranking")
