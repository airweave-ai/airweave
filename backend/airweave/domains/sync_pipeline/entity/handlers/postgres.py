...
class EntityPostgresHandler(EntityActionHandler):
    ...
    async def _do_inserts(
        ...
        await self._entity_repo.bulk_create(db, objs=create_objs, ctx=sync_context)
    ...
    async def _do_updates(
        ...
        await self._entity_repo.bulk_update_hash(db, rows=update_pairs)
    ...
    async def _do_deletes(
        ...
        await self._entity_repo.bulk_remove(db, ids=db_ids, ctx=sync_context)
    ...
    async def _fetch_existing_map(
        ...
        return await self._entity_repo.bulk_get_by_entity_sync_and_definition(
