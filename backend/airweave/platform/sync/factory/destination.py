...
class DestinationBuilder:
    ...
    async def _get_active_ids(self, sync: schemas.Sync) -> list[UUID]:
        ...
        Respects destination roles if sync_connection CRUD supports it:
        - ACTIVE: receives writes + serves queries
        - SHADOW: receives writes only (migration testing)
        - DEPRECATED: skipped (no writes)

        Falls back to all destination_connection_ids if role-based filtering
        is not available (pre-multiplexer).
        """
        # Try to use role-based filtering if available
        try:
            if hasattr(crud, "sync_connection") and hasattr(
                crud.sync_connection, "get_active_and_shadow"
            ):
            ...
            self.logger.debug(f"Role-based destination filtering not available: {e}")

        # Fallback: use all destination connection IDs
