...
class EntityPipeline:
    ...
    async def process(
        ...
        for i, entity in enumerate(entities_to_process, 1):
            ...
            print(entity.textual_representation[:1000])
            if len(entity.textual_representation) > 1000:
                print("...")
            print("-" * 80)
        ...
        if chunk_entities:
            ...
            for chunk_entity in chunk_entities:
                ...
                if chunk_entity.entity_id != current_entity_id:
                    ...
                    # Print entity header for first chunk
                    print(f"\n{'─' * 80}")
                    print(f"Entity: {chunk_entity.name}")
                    print(f"Type: {chunk_entity.__class__.__name__}")
                    print(f"{'─' * 80}")
                else:
                    entity_chunk_count += 1
                ...
                if entity_chunk_count <= 3:
                    print(f"\n[Chunk {chunk_idx}] ~{token_count} tokens")
                    print("┌" + "─" * 78 + "┐")
                    # Show first 500 chars with actual line breaks (max 10 lines)
                    preview_lines = chunk_entity.textual_representation[:500].split("\n")[:10]
                    for line in preview_lines:
                        # Truncate long lines
                        display_line = line[:76] if len(line) > 76 else line
                        print(f"│ {display_line:<76} │")
                    if len(chunk_entity.textual_representation) > 500:
                        print(f"│ {'... (truncated)':<76} │")
                    print("└" + "─" * 78 + "┘")
                    displayed += 1

            if len(chunk_entities) > 10:
                print(f"\n{'=' * 80}")
                print(f"... and {len(chunk_entities) - 10} more chunk entities")
                print("=" * 80)
        ...
        await self._embed_entities(entities_to_process, sync_context)
    ...
    async def _cleanup_processed_files(
        ...
        for entity in entities_to_clean:
            ...
            if not isinstance(entity, (FileEntity, CodeFileEntity)):
                continue
    ...
    async def cleanup_temp_files(self, sync_context: SyncContext) -> None:
        ...
        try:
            ...
            if not hasattr(sync_context.source, "file_downloader"):
                sync_context.logger.debug("No file downloader configured, skipping temp cleanup")
                return
