...
LEGACY CODE - REPLACED BY EntityPipeline
This file is kept for reference and backwards compatibility.
The EntityPipeline class in entity_pipeline.py is the replacement.
"""
...
from airweave.platform.entities._base_legacy import (
    BaseEntity,
    DestinationAction,
    PolymorphicEntity,
)
...
class EntityProcessor:
    ...
    async def process_batch(
        ...
        *,
        ...
        children_by_parent = await self._transform_parents(
            to_transform, sync_context, inner_concurrency
        )
    ...
    async def _transform_parents(
        ...
        async def _do_transform(p: BaseEntity):
            try:
                return p.entity_id, await self._transform(p, sync_context)
            except asyncio.CancelledError:
    ...
    async def _enrich(self, entity: BaseEntity, sync_context: SyncContext) -> BaseEntity:
        ...
        from airweave.platform.entities._base_legacy import AirweaveSystemMetadata
