...
class SharePoint2019SiteEntity(BaseEntity):
    """Schema for a SharePoint 2019 site (web/subsite).

    Sites (also called webs) are containers for lists, libraries, and other content.
    A site collection has a root site and can have multiple sub-sites.

    Reference:
        https://docs.microsoft.com/en-us/sharepoint/dev/sp-add-ins/working-with-webs-using-rest
    """

    id: str = AirweaveField(
        ...,
        description="Site ID (GUID).",
        is_entity_id=True,
    )
    title: str = AirweaveField(
        ...,
        description="The displayable title of the site.",
        embeddable=True,
        is_name=True,
    )
    description: Optional[str] = AirweaveField(
        None, description="The description of the site.", embeddable=True
    )
    ...
    server_relative_url: Optional[str] = AirweaveField(
        None, description="Server-relative URL of the site.", embeddable=False
    )
    web_template: Optional[str] = AirweaveField(
        None,
        description="The web template used for the site (e.g., 'STS', 'WIKI').",
        embeddable=False,
    )
    language: Optional[int] = AirweaveField(
        None, description="The LCID (locale ID) of the site.", embeddable=False
    )
    ...
    def web_url(self) -> str:
        """Return the URL to view the site in browser."""
        return self.url
...
class SharePoint2019ListItemEntity(BaseEntity):
    ...
    file_system_object_type: Optional[int] = AirweaveField(
        None, description="Type: 0=File, 1=Folder, 2=Web.", embeddable=False
    )
    server_relative_url: Optional[str] = AirweaveField(
...
class SharePoint2019FileEntity(FileEntity):
    ...
    length: Optional[int] = AirweaveField(None, description="File size in bytes.", embeddable=False)
