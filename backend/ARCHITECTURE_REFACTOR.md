...
|----------|----------|--------------|--------------|
| **Unit** | `<domain>/tests/test_logic.py` | Pure functions | None |
| **Integration** | `<domain>/tests/test_operations.py` | Operations | Fake protocols |
| **Cross-Domain** | `tests/cross_domain/` | Multi-domain flows | Fake protocols |
| **E2E** | `tests/e2e/` | Full API | TestClient |

### Layered conftest.py

```
backend/
├── conftest.py                           # Root: db, all fakes
└── airweave/domains/billing/tests/
    └── conftest.py                       # Domain: billing-specific fixtures
```
...
testpaths = ["airweave", "tests"]  # Discover colocated + cross-cutting
...
# Run one domain
pytest airweave/domains/billing/

# Skip E2E (fast local dev)
