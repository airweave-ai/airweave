"""
E2E smoke tests for spotlight search filtering.

Uses stub + optional Stripe data to validate that spotlight filters:
...
3. Survive merging with planner-generated filters in agentic mode
...
def _cleanup_cached_collection():
    ...
    """
    global _cached_spotlight_collection, _cached_spotlight_readable_id

    if _cached_spotlight_collection is None:
        return
    ...
    try:
        ...
        with httpx_sync.Client(
            ...
            for conn_id in _cached_spotlight_collection.get("_connections_to_cleanup", []):
                try:
                    client.delete(f"/source-connections/{conn_id}")
                except Exception:
                    pass

            if _cached_spotlight_readable_id:
                try:
                    client.delete(f"/collections/{_cached_spotlight_readable_id}")
                except Exception:
                    pass
    except Exception:
        pass
...
def print_results_summary(results: dict, test_name: str, filter_list: list = None):
    """Print a summary of spotlight search results for debugging."""
    print(f"\n{'='*80}")
...
_cached_spotlight_collection: Optional[Dict] = None
_cached_spotlight_readable_id: Optional[str] = None
...
async def _create_spotlight_collection(client: httpx.AsyncClient) -> Dict:
    """Create a collection with stub + optional Stripe data for spotlight filter testing.
    ...
    # Create collection
    collection_data = {"name": f"Spotlight Filter Test {int(time.time())}"}
    ...
    stub_connection_data = {
        "name": f"Stub Spotlight Filter Test {int(time.time())}",
        "short_name": "stub",
    ...
    # Verify stub data is searchable via spotlight (direct mode for speed)
    stub_verified = False
    ...
    if stripe_api_key and stripe_api_key != "sk_test_dummy":
        stripe_connection_data = {
            "name": f"Stripe Spotlight Filter Test {int(time.time())}",
            "short_name": "stripe",
            "readable_collection_id": readable_id,
    ...
    print(f"\n  Spotlight filter collection ready. Sources: {result['sources']}\n")
...
@pytest_asyncio.fixture(scope="function")
async def spotlight_filter_collection(
    spotlight_filter_client: httpx.AsyncClient,
) -> AsyncGenerator[Dict, None]:
    """Provide collection with stub + optional Stripe data for spotlight filter testing.

    Uses module-level cache to avoid recreating collections for each test.
    """
    global _cached_spotlight_collection, _cached_spotlight_readable_id

    client = spotlight_filter_client

    if _cached_spotlight_collection is not None and _cached_spotlight_readable_id is not None:
        check_response = await client.get(f"/collections/{_cached_spotlight_readable_id}")
        if check_response.status_code == 200:
            yield _cached_spotlight_collection
            return

    _cached_spotlight_collection = await _create_spotlight_collection(client)
    _cached_spotlight_readable_id = _cached_spotlight_collection["readable_id"]
...
async def test_invalid_field_name_rejected(
    spotlight_filter_client: httpx.AsyncClient,
    spotlight_filter_collection: Dict,
):
...
async def test_search_without_filter_returns_results(
    ...
    """Baseline: spotlight search without filter returns results."""
    results = await do_spotlight_search(
        spotlight_filter_client, spotlight_filter_collection["readable_id"], "stub"
    )
...
async def test_filter_by_source_name_equals(
    ...
    results = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "test",
...
async def test_filter_by_source_name_stripe_only(
    ...
    if not spotlight_filter_collection.get("has_stripe"):
...
async def test_filter_multi_source_excludes_correctly(
    ...
    stub_results = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "test",
    ...
    stripe_results = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "test",
...
async def test_filter_reduces_result_count(
    ...
    """
    unfiltered = await do_spotlight_search(
        spotlight_filter_client, spotlight_filter_collection["readable_id"], "stub"
    )
    ...
    filtered = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "stub",
        filter_list=filter_list,
...
async def test_user_filter_enforced_in_agentic_mode(
    ...
    """Test that user-supplied filters are enforced in agentic mode.

    In agentic mode, the planner generates its own filters which are merged
    with user filters via SpotlightCompletePlanBuilder. This test proves
    ...
    for result in results["results"]:
        ...
        assert source_name == "stub", (
            f"User filter not enforced in agentic mode: expected stub, got {source_name}"
        )
...
async def test_user_filter_exclusion_in_agentic_mode(
    ...
    """Test that a user filter for a non-existent source returns 0 results in agentic mode.
    ...
    assert "results" in results
    assert len(results["results"]) == 0, (
        f"Expected 0 results for non-existent source in agentic mode, "
...
async def test_user_filter_in_agentic_vs_direct_consistency(
    ...
    """Test that user filters are respected in both agentic and direct mode.

    Both modes should enforce the user filter. This verifies the merge path
    in agentic mode produces the same filtering behavior as direct mode.
    """
    filter_list = [
        {
            "conditions": [
    ...
    direct_results = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "test data",
        filter_list=filter_list,
    ...
    agentic_results = await do_spotlight_search(
        spotlight_filter_client,
        spotlight_filter_collection["readable_id"],
        "test data",
        filter_list=filter_list,
    ...
    for result in direct_results.get("results", []):
        source_name = result.get("airweave_system_metadata", {}).get("source_name")
        assert source_name == "stub", f"Direct mode: expected stub, got {source_name}"

    for result in agentic_results.get("results", []):
        source_name = result.get("airweave_system_metadata", {}).get("source_name")
        assert source_name == "stub", f"Agentic mode: expected stub, got {source_name}"
...
async def test_planner_and_user_filter_combined(
    ...
    This proves SpotlightCompletePlanBuilder.build() correctly ANDs planner
