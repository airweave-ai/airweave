"""
Async test module for Spotlight Search functionality.

Tests the spotlight agentic search endpoint:
- Agentic mode (default, multi-step search)
- Direct mode (single-pass search)
...
class TestSpotlightSearch:
    """Test suite for spotlight search functionality.
    ...
    @pytest.mark.asyncio
    async def test_spotlight_search_agentic_mode(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test spotlight search with default agentic mode.

        Agentic mode runs a multi-step plan-search-evaluate loop.
        ...
        assert response.status_code == 200, f"Spotlight search failed: {response.text}"

        data = response.json()
        assert "results" in data
        assert isinstance(data["results"], list)
        assert len(data["results"]) > 0, "Agentic mode should return results from Stripe data"
    ...
    @pytest.mark.asyncio
    async def test_spotlight_search_direct_mode(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test spotlight search with explicit direct mode.

        Direct mode performs a single search pass without the evaluate loop.
        ...
        assert response.status_code == 200, f"Direct mode search failed: {response.text}"

        data = response.json()
        assert "results" in data
        assert isinstance(data["results"], list)
        assert len(data["results"]) > 0, "Direct mode should return results from Stripe data"
        assert "answer" in data
        assert "text" in data["answer"]
        assert "citations" in data["answer"]
    ...
    @pytest.mark.asyncio
    async def test_spotlight_response_structure(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Deep validation of spotlight response structure.
    ...
    @pytest.mark.asyncio
    async def test_spotlight_search_returns_results_from_synced_data(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test that spotlight search returns results from synced Stripe data.
    ...
    @pytest.mark.asyncio
    async def test_spotlight_empty_query_returns_422(
        ...
        response = await api_client.post(
            f"/collections/{collection['readable_id']}/spotlight/search",
            json=payload,
        )
    ...
    @pytest.mark.asyncio
    async def test_spotlight_invalid_mode_returns_422(
        self, api_client: httpx.AsyncClient, collection: Dict
    ):
    ...
    @pytest.mark.asyncio
    async def test_spotlight_stream_returns_sse_events(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
    ...
    @pytest.mark.asyncio
    async def test_spotlight_stream_direct_mode(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test streaming endpoint in direct mode.

        Direct mode should still stream events and end with a 'done' event,
        ...
        # Direct mode should have planning + searching events but no evaluating
