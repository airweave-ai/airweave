...
class TestAgenticSearch:
    ...
    @pytest.mark.asyncio
    async def test_agentic_search_with_explicit_model(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test agentic search with a non-default model.

        Verifies the endpoint accepts an explicit model choice (gpt-oss-120b instead
        of the default zai-glm-4.7) and still returns a valid response.
        """
        payload = {
            "query": "invoices",
            "mode": "fast",
        ...
        assert response.status_code == 200, f"Explicit model search failed: {response.text}"

        data = response.json()
        assert "results" in data
        assert isinstance(data["results"], list)
        assert len(data["results"]) > 0, "Explicit model should return results"
        assert "answer" in data
        assert "text" in data["answer"]

    @pytest.mark.asyncio
    async def test_agentic_search_invalid_model_returns_422(
        self, api_client: httpx.AsyncClient, collection: Dict
    ):
        """Test that an invalid model value returns 422 Unprocessable Entity.
    ...
    @pytest.mark.asyncio
    async def test_agentic_search_without_model_uses_default(
        self, api_client: httpx.AsyncClient, module_source_connection_stripe: dict
    ):
        """Test that omitting the model parameter uses the default (zai-glm-4.7).

        Should return results and answer normally.
        """
        payload = {
            "query": "invoices",
            "mode": "fast",
        ...
        assert response.status_code == 200, f"Default model search failed: {response.text}"

        data = response.json()
        assert "results" in data
        assert len(data["results"]) > 0, "Default model should return results"
